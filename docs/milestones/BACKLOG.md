...
| 16 | **The mutex poison branches in the appender.** ⚠️ Found by M6e's coverage criterion, which measured `pstore-catalog` at **94.90%** against the 95% it asked for. `append.rs` is 88.75%, and the gap is the lock-poison arms that degrade to "record unconditionally" rather than panicking — reachable only by a thread that panics while holding a private lock. The criterion was left as written and the number reported rather than moved to match it. | [M6e](M6e/VERIFIED.md) | S |
