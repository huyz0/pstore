...
| ~~OQ-150~~ | ~~Which backend has the highest CAS fidelity for day-to-day dev?~~ **ANSWERED — Azurite, and the stated obstacle was not the real one.** Both emulators pass `create_if_absent` and `compare_and_swap`; MinIO's missing wildcard is fixed ([C-13](../09-rust-stack/dev-and-test-environment.md)). MinIO fails `aba_resistance` — content-derived ETags — and Azurite passes it, so on the CAS axis Azurite is 3/3 against 2/3. ⚠️ Azurite is **not** therefore the better dev target: it has no suffix range at all ([C-14](../02-object-storage/request-efficiency-patterns.md)), which costs a round trip on every cold open. | [`profiles/capability-matrix.md`](../../profiles/capability-matrix.md) |
| ~~**OQ-153**~~ | **ANSWERED — no, and worse than unreachable.** Asked directly over the JSON upload path (`scripts/conformance.sh --gcs-precondition`), `fake-gcs-server` **accepts `ifGenerationMatch` and ignores it**: create-if-absent against an existing object returns **200 and overwrites**, and so does a compare-and-swap on a stale generation. ⚠️ That is the worst shape a precondition can have — the same one MinIO's wildcard had ([C-13](../09-rust-stack/dev-and-test-environment.md)) — because a CAS built on it reports success and loses the write, where a backend that *refused* the parameter would be detectable. ⚠️ The `object_store` XML-routing obstacle was masking this, and a newer image does not help: **1.52 and 1.55 behave identically**. Two of the three exits this question named are therefore closed, and the third — a different client — would only reach a precondition that is not enforced. | [`profiles/capability-matrix.md`](../../profiles/capability-matrix.md) |
...
~~`OQ-167`~~ **closed** — `scripts/check-verified.py`, 9 selftest cases. Catches missing evidence, drifted test names, unevidenced claims and ghost criteria; accepts `NOT-RUN`/`OBSERVED-NOT` and in-progress milestones. Cannot see macro-generated tests, cannot check the evidence is *true*, and cannot see PARTIAL drift — one resolving name rescues a drifted sibling, because refusing every backticked snake_case word would refuse the methods and probes ledgers legitimately mention ·
