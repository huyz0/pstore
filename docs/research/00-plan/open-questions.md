...
| **OQ-153** | Is `fake-gcs-server` faithful to `ifGenerationMatch`? **STILL OPEN, and the obstacle is now named:** the question is unreachable through `object_store`. Its GCS client writes with the XML API (`PUT /{bucket}/{object}`); `fake-gcs-server` 1.52 routes that path to its JSON upload handler and answers `400 invalid uploadType`, so no request gets close enough to carry a precondition. Reads over the XML path do work. To close it: a newer `fake-gcs-server`, a different client, or the JSON upload path. | [`profiles/capability-matrix.md`](../../profiles/capability-matrix.md) |
