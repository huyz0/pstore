...
Every doc states which research question(s) it answers. Question IDs (`Q1`…`Q48`) are
...
| Doc | Answers | Status |
|---|---|---|
| [research-plan.md](00-plan/research-plan.md) | — | The plan: 48 questions, 9 phases, method. |
| [open-questions.md](00-plan/open-questions.md) | D36 | **168 open questions, risk-ranked.** Tier 1 is what could invalidate the architecture. |
...
| [routing-and-placement.md](04-cluster/routing-and-placement.md) | Q12 | **Rendezvous hashing is O(N) and disqualified above ~100 nodes.** Use Local Rendezvous Hashing (O(C)) + bounded-load skipping. | **⚠️ C-6 from M4a: C≈32 must grow with N (1.81× imbalance at N=2000 if fixed), and the 10–15% balance figure is not reproduced — measured 1.21× at best.**
...
| [quantization.md](06-indexing/quantization.md) | Q20 | **RaBitQ over PQ**: no per-tenant training (decisive at millions of tenants) and a real error bound (PQ has none and fails badly on some data). 1-bit = 96 GB per 1B vectors. **⚠️ C-3 from M3: rung 0 alone gives 0.30 recall@10, not the 90–95% claimed — int8 rerank shares its round trip and gives 0.981.** |
...
| **[spec-driven-development.md](09-rust-stack/spec-driven-development.md)** | Q48 | Surveyed Spec Kit (~800 lines/feature), BMAD (12+ agent personas), OpenSpec (~250, delta-based). **The corpus is already the spec** — Spec Kit here would restate it. Adopt the sibling repo's 23-line standard + OpenSpec's delta idea. **A strong model fails invisibly, so the verification ledger matters more than the spec.** |
