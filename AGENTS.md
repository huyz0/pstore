...
| `scripts/mutants.sh` | D-111 — the gate that makes coverage mean something. **Incremental by default**: a full sweep is **3,339 mutants** (462 in M5) and one nightly shard of ~420 takes about two hours, so the bare command tests only what this branch changed, and the full sweep is nightly and sharded. Binary entry points are never mutated -- no test can run one -- but `ships = false` crates are: they are testable, and hold the conformance probes (M8d) |
...
| `scripts/check-portable.sh` | every shell script under `scripts/` runs on Linux, macOS and Windows, or declares `# portable: no` -- refused on anything `gates.sh` runs. ⚠️ `gates.sh` ran on one OS for seven milestones: on Windows every Python gate failed (`python3` is the Store stub) and rustc refused the repo's own `split-debuginfo`. Rung 3; `lib/py.sh` beside it is the one way a script runs Python |
| `scripts/gates.sh` | **all of the fast gates, in one command** -- and CI's `portable` job runs it on `ubuntu-latest`, `windows-latest` and `macos-latest` (under `/bin/bash` 3.2), so "works on my machine" is a failing job rather than a surprise |
