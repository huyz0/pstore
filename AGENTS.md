...
| `scripts/mutants.sh` | D-111 — the gate that makes coverage mean something. **Incremental by default**: a full sweep is **3,401 mutants** (462 in M5) and one nightly shard of 426 takes about two hours, so the bare command tests only what this branch changed, and the full sweep is nightly and sharded |
