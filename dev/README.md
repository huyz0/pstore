...
| **Two containers, one bucket** | `scripts/byoc.sh` | ⚠️ **The only test here that crosses a process boundary.** Builds `dev/Dockerfile.server`, runs two servers on two lanes against one RustFS, and asserts that a document written and folded through one is returned by the other — and returns 404 before the fold. Also the image's refusal arms (unprobed profile, missing lane) and the `docs/deploy.md` gate. ⚠️ Outside `cargo test`, and outside AGENTS.md's Gates table, because it needs Docker and CI has none |
